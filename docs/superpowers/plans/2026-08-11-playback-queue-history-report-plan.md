...
- [ ] **Step 4: 实现历史服务和原子持久化**
  ...
  `startSession` 创建单个会话，`recordPosition` 记录最大已听位置；`endSession` 以 `min(30 秒, duration * 0.5)` 计算“30 秒或 50% 先达到”的资格门槛，合格才写入事件，并清理 12 个月前记录。暂停后结束当前会话，恢复播放时建立新会话；写入失败只记录日志并保留内存状态。
