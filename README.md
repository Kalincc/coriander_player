...
- 新增持久化播放队列：支持插入下一首、追加到队尾、移除、清空、重排、随机播放恢复和失效文件清理。
...
- 新增增量扫描与安全迁移：按文件路径、修改时间和大小减少重复标签读取；旧索引需要执行一次完整重建，扫描失败时不会用不完整索引覆盖现有数据。
- 本地播放队列、历史、歌单和报告数据保存在 `Documents\\coriander_player`，更新或重新安装播放器不会自动删除这些数据。
